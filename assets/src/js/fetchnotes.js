...
Database = (function() {
  ...
  */
  ...
  Database.prototype.callAsync = function(calls) {
    var collection, currentCall, error, method, model, options, string, success, _ref;
    if (calls.length) {
      currentCall = calls[0];
      string = currentCall.string, model = currentCall.model, options = currentCall.options;
      options || (options = {});
      success = options.success, error = options.error;
      _ref = string.split('.'), collection = _ref[0], method = _ref[1];
      options.success = function(arg) {
        if (success) success(arg);
        console.log("Success calling " + string + ", calling next method!");
        return callAsync(calls.slice(1));
      };
      options.error = function(arg) {
        if (error) error(arg);
        console.error(arg);
        console.error("Error calling " + string + ", calling next method anyway");
        return callAsync(calls.slice(1));
      };
      return this[collection][method](model, options);
    }
  };
